Feature: Auto tests of the Demo Site
  ...
  Scenario: Do a movie search and continue to it's details page and then back to the front page
    Given that we navigate to localhost
    And that we know the element with the "id" named "search-field" as "input"
    And input "Fargo" to the element "input"
    And that we know the element with the "link-text" named "More info" as "button"
  ...
  Scenario: Do a series search and continue to it's details page, and then to the season page, and then back to the front page
    Given that we navigate to localhost
    And that we know the element with the "model" named "type" as "radio"
    And click the 2nd element "radio"
    And that we know the element with the "id" named "search-field" as "input"
    ...
    And that we know the element with the "model" named "season" as "season-search"
    And input "1" to the element "season-search"
    ...
    And that we know the element with the "repeater" named "episode in seasonData.Episodes" as "episodes"
  ...
  Scenario: Watch my favorite movies
    Given that we navigate to localhost
    And that we know the element with the "options" named "types.name for types in types.availableTypes track by types.id" as "favorites"
  ...
  Scenario: Watch my favorite series
    ...
    And that we know the element with the "repeater" named "movie in movies" as "series"
